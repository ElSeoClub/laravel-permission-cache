...
use Elseoclub\Permission\Exceptions\RoleDoesNotExist;
use Elseoclub\Permission\Tests\TestModels\Admin;
use Elseoclub\Permission\Tests\TestModels\SoftDeletingUser;
...
class HasRolesTest extends TestCase {
    ...
    public function it_can_assign_and_remove_a_role_using_enums() {
        ...
        $this->assertTrue( $this->testUser->hasAllRoles( [
            'user-manager',
            'writer',
            'casted_enum-1',
            'casted_enum-2'
        ...
        $this->assertFalse( $this->testUser->hasAllRoles( [
            'user-manager',
            'writer',
            'casted_enum-1',
            'casted_enum-2',
        ...
        $this->assertTrue( $this->testUser->hasExactRoles( [
            'user-manager',
            'writer',
            'casted_enum-1',
            'casted_enum-2'
    ...
    public function it_can_determine_that_a_user_has_exact_all_of_the_given_roles() {
        ...
        $this->assertFalse( $this->testUser->hasExactRoles( [
            'testRole',
            'second role',
            'third role'
        ], 'fakeGuard' ) );
