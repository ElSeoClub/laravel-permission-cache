...
trait HasRolesCache {
    ...
    public function assignRole( ...$roles ) {
        ...
        if ( $model->exists ) {
            $instanceRolesCache = $this->loadMissingRolesCache();
            $currentRoles       = $instanceRolesCache->map( fn( $role ) => $role->getKey() )->toArray();
    ...
    public function hasRole( $roles, ?string $guard = null ): bool {
        ...
        if ( is_int( $roles ) || PermissionRegistrar::isUid( $roles ) ) {
            ...
            return $guard
                ? $instanceRolesCache->where( 'guard_name', $guard )->contains( $key, $roles )
                : $instanceRolesCache->contains( $key, $roles );
        }

        if ( is_string( $roles ) ) {
            return $guard
                ? $instanceRolesCache->where( 'guard_name', $guard )->contains( 'name', $roles )
                : $instanceRolesCache->contains( 'name', $roles );
        }

        if ( $roles instanceof Role ) {
            return $instanceRolesCache->contains( $roles->getKeyName(), $roles->getKey() );
        }
        ...
        if ( $roles instanceof Collection ) {
            return $roles->intersect( $guard ? $instanceRolesCache->where( 'guard_name', $guard ) : $instanceRolesCache )->isNotEmpty();
        }
    ...
    public function hasAllRoles( $roles, ?string $guard = null ): bool {
        ...
        $roleNames = $guard
            ? $instanceRolesCache->where( 'guard_name', $guard )->pluck( 'name' )
            : $this->getRoleNames();
    ...
    public function hasExactRoles( $roles, ?string $guard = null ): bool {
        ...
        return $instanceRolesCache->count() == $roles->count() && $this->hasAllRoles( $roles, $guard );
    }
    ...
    public function getRoleNames(): Collection {
        ...
        return $instanceRolesCache->roles->pluck( 'name' );
    }
    ...
    public function loadMissingRolesCache() {
        return Cache::tags( get_class( $this ) . ':' . $this->id )->rememberForever( 'roles', function () {
            return $this->loadMissing( 'roles' )->roles;
        } );
    }

    public function forgetCacheInstanceRoles(): void {
        $cacheTag = get_class( $this ) . ':' . $this->id;
        $cacheKey = 'roles';
